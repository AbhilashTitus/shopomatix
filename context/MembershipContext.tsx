...
import { UserMembership, ShopomatixCoin } from '@/lib/types/membership';
import { useAuth } from './AuthContext';

interface MembershipContextType {
  membership: UserMembership | null;
  coinBalance: ShopomatixCoin | null;
  isLoading: boolean;
...
export function MembershipProvider({ children }: { children: React.ReactNode }) {
  ...
  const [coinBalance, setCoinBalance] = useState<ShopomatixCoin | null>(null);
  ...
  const mockCoinBalance: ShopomatixCoin = {
    id: 'coin_1',
    userId: user?.id || '1',
    balance: 1250,
    totalEarned: 3500,
